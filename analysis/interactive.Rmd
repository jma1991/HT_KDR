...
# Create iSEE object
iSEE(sce, appTitle = "HT_KDR", colormap = ecm)
```

Deploy the iSEE application to the shiny server:

```{r}
deployApp(appDir = "shiny/sample", appName = "HT_KDR", appTitle = "HT_KDR", launch.browser = FALSE, forceUpdate = TRUE)
```

Allocate more memory to the iSEE application:

```{r}
configureApp(appName = "HT_KDR", appDir = "shiny/sample", size = "xxlarge")
```
