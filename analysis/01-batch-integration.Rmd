...
```{r}
sce <- list(
  HT_KDR_HET_A = readRDS("data/HT_KDR_HET_A.rds"),
  HT_KDR_HOM_B = readRDS("data/HT_KDR_HOM_B.rds")
)
```
...
```{r}
dec <- list(
  HT_KDR_HET_A = metadata(sce$HT_KDR_HET_A)$modelGeneVarByPoisson,
  HT_KDR_HOM_B = metadata(sce$HT_KDR_HOM_B)$modelGeneVarByPoisson
)
```
...
```{r}
ind <- c("HT_KDR_HET_A", "HT_KDR_HOM_B")
```
